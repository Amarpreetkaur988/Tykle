...
import com.tykle.R;
import com.tykle.adapter.AttachmentPreviewAdapter;
import com.tykle.adapter.ChatAdapter;
import com.tykle.util.chat.ChatHelper;
import com.tykle.util.qb.PaginationHistoryListener;
import com.tykle.util.qb.QbChatDialogMessageListenerImp;
import com.tykle.util.qb.QbDialogHolder;
import com.tykle.util.qb.QbDialogUtils;
import com.tykle.util.qb.VerboseQbChatConnectionListener;
import com.tykle.widget.AttachmentPreviewAdapterView;
...
public class ChatActivity extends BaseActivity implements OnImagePickedListener, View.OnClickListener {
    ...
    private void sendNotification(String text, String attachemnt) {
        ...
        if (HelperClass.isNetworkConnected(ChatActivity.this)) {

            ApiClient.getApiClient();

            ApiService apiService = ApiClient.retrofit.create(ApiService.class);

            apiService.SendMessage(HelperClass.getQuickBoxid(ChatActivity.this), qbChatDialog.getRecipientId() + "", text, attachemnt).enqueue(new Callback<Map>() {
                @Override
    ...
    private void Action(String s) {
        ...
        if (HelperClass.isNetworkConnected(ChatActivity.this)) {
            ...
            apiService.GoOnlineOffline("1", s,
